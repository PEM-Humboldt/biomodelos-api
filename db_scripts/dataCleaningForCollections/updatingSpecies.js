...
db.species.updateMany({"$or": [{"bmClass": {"$exists": false}}, {"bmClass": {"$nin": ["Plantas", "Invertebrados", "Aves", "Peces","Reptiles", "Mamiferos", "Anfibios"]}}]}, {"$set": {"bmClass": null}});
...
db.species.updateMany({"$or": [{"TaxVerifSource": {"$exists": false}}, {"TaxVerifSource": {"$in": ["","[\"NA\"]", "NA", "[]"]}}]}, {"$set": {"TaxVerifSource": null}});
